...
let abi = JSON.parse(fs.readFileSync("./build/TruebitMapReduceContract.abi"));
let bin = fs.readFileSync("./build/TruebitMapReduceContract.bin");
...
async function deploy() {
  ...
  let randomFile;
  try {
    randomFile = await addRandomIPFSFile(
      tbFileSystem,
      account,
      "_dev_urandom",
      fs.readFileSync("_dev_urandom")
    );
  } catch (e) {
  ...
  let fileNonce = Math.floor(Math.random() * Math.pow(2, 30));
  let codeFileID = await tbFileSystem.methods
    .calculateId(fileNonce)
    .call({ from: account });
  let args = [
  ...
  await tbFileSystem.methods
    .addIpfsFile(name, size, ipfsHash, mr, fileNonce)
    .send({ from: account, gas: 300000 });
  await tbFileSystem.methods
    .setCodeRoot(fileNonce, codeRoot, 1, 20, info.memsize, 8, 20, 10)
    .send({ from: account, gas: 300000 });
  console.log("Registered codefile with Truebit filesystem");
  console.log(
    await tbFileSystem.methods.vmParameters(codeFileID).call({ from: account })
  );

  // Deploy mapReduce contract
